...
class GPT2Model(GPTPreTrainedModel):
    ...
    def embed(self, input_ids):
        ...
        embedding = inputs_embeds + pos_embeds

        final_embedding = self.embed_dropout(embedding)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        last_non_pad_idx = attention_mask.sum(
            dim=1) - 1  # Subtract 1 to get last index
    ...
    def from_pretrained(cls, model='gpt2', d=768, l=12, num_heads=12):
        ...
        for i in range(l):
            ...
            l.interm_dense.bias.data = gpt_model.state_dict()[
                f'h.{i}.mlp.c_fc.bias']
            l.out_dense.weight.data = gpt_model.state_dict()[
                f'h.{i}.mlp.c_proj.weight'].T
            l.out_dense.bias.data = gpt_model.state_dict()[
                f'h.{i}.mlp.c_proj.bias']

            # Remap second layer norm weights.
            l.out_layer_norm.weight.data = gpt_model.state_dict()[
                f'h.{i}.ln_2.weight']
            l.out_layer_norm.bias.data = gpt_model.state_dict()[
                f'h.{i}.ln_2.bias']

        # Remap the final layer norm values.
        our_model.final_layer_norm.weight.data = gpt_model.state_dict()[
            'ln_f.weight']
        our_model.final_layer_norm.bias.data = gpt_model.state_dict()[
            'ln_f.bias']
