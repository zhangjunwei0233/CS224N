...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(
            kernel_size=2, in_channels=embed_size, out_channels=embed_size, padding='same')

        # receive src word vec (h,  1), concatenate bidirectional hidden layer, output hidden layers (2 * h, 1)
        self.encoder = nn.LSTM(input_size=embed_size,
                               hidden_size=self.hidden_size, bias=True, bidirectional=True)

        # receive y_{t-1} word concatenated with o_{t-1}, with hidden layer size h_t and cell c_t, predict y_t
        self.decoder = nn.LSTMCell(
            input_size=embed_size + self.hidden_size, hidden_size=self.hidden_size, bias=True)

        # project bidirection LSTM's output hidden layer (2 * h, 1) to unidirection input hidden layer (h, 1)
        self.h_projection = nn.Linear(
            in_features=2 * self.hidden_size, out_features=self.hidden_size, bias=False)

        # project bidirection LSTM's cell layer (2 * h, 1) to unidirection input cell layer (h, 1)
        self.c_projection = nn.Linear(
            in_features=2 * self.hidden_size, out_features=self.hidden_size, bias=False)

        # project encoder hidden layer (values) and decoder hidden layer (query) to form attention score
        self.att_projection = nn.Linear(
            in_features=2 * self.hidden_size, out_features=self.hidden_size, bias=False)

        # project decoder hidden layer concatenated with attention (3 * h, 1) to output vector
        self.combined_output_projection = nn.Linear(
            in_features=3 * self.hidden_size, out_features=self.hidden_size, bias=False)

        # project the output vector to probability distribution among vocab
        self.target_vocab_projection = nn.Linear(
            in_features=self.hidden_size, out_features=len(self.vocab.tgt), bias=False)

        # add a dropout layer to the output vector o
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        # Construct Tensor X (src_len, b, e) from source_padded (src_len, b)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # split 'Y' into Y_t and iterate through
        Y_ts = torch.split(Y, 1, 0)
        for Y_t in Y_ts:
            # concatenate with o_prev to form Ybar_t
            # (1, b, e) -> (b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Apply decoder to Ybar_t and dec_statc to form new dec_state
        dec_state = self.decoder(Ybar_t, dec_state)
        (dec_hidden, dec_cell) = dec_state

        # Compute attention score using dec_hidden
        e_t = torch.bmm(enc_hiddens_proj,
                        dec_hidden.unsqueeze(dim=2)).squeeze(dim=2)
        ...
        # apply softmax to e_t to obtain alpha_t
        alpha_t = F.softmax(e_t, dim=1)

        # obtion the attention vector
        a_t = torch.bmm(alpha_t.unsqueeze(dim=1), enc_hiddens).squeeze(dim=1)

        # Concatenate dec_hidden and a_t to compute U_t (b, 3h)
        U_t = torch.cat([a_t, dec_hidden], dim=1)

        # apply combined output projection to produce V_t
        V_t = self.combined_output_projection(U_t)

        # Compute O_t using V_t
