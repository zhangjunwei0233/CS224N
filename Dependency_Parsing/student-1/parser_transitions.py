...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        # create a minibatch and make prediction using model
        minibatch = unfinished_parses[:batch_size]
        transitions = model.predict(minibatch)
        to_remove = set()

        # execute predicted transitions on each parse
        for i, partial_parse in enumerate(minibatch):
            ...
            if not partial_parse.buffer and len(partial_parse.stack) == 1:
                to_remove.add(partial_parse)

        # remove finished parses
        unfinished_parses = [
            p for p in unfinished_parses if p not in to_remove]
